...
When hosted, sync is read-driven with a scheduled floor. Any read — an app over HTTP or an agent over MCP — warms the site: if its ledger hasn't been reconciled within the reconciliation window (6h), the read kicks a background sync and still returns current data immediately, so a warm site is never more than a few hours stale. Concurrent or in-flight syncs coalesce into a no-op, so bursty agent traffic can't stack jobs. On boot the server force-refreshes every configured site once (sequentially), so a deploy always lands current data instead of waiting for the first read. A per-site daily scheduled task (see [docs/deploy.md](docs/deploy.md)) remains the cold-start floor for sites nothing reads between deploys. Opening the TUI or pressing `r` forces a sync regardless of the window.
