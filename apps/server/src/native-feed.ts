...
//
// Ported from the legacy `src/native-feed.ts`: the grammar and every rendered
// byte are unchanged; the data now comes from the Effect domain services
// (Storage/Registry/Sitemap/Reports/CurrentSite) instead of the legacy modules,
// and the shared presentation copy is imported from the Reports domain (single
// source of truth). `feedFor` is an Effect run through the per-site runtime.
//
...
import {
  ...
  signalMeaning,
  signalReason,
} from "@rp/domain/reports/reports"
import { type LogFeedEntry, type LogReadout } from "@rp/domain/reports/schema"
...
const document = (
  ...
  const lines = [
    line(["header", `${summary.rows} rows · ${summary.dates} days · ${debug ? "DEBUG" : "LIVE"}`]),
...
const logView = Effect.gen(function* () {
  ...
  return document(summary, debug, meta, ["DATE", "KIND", "TARGET", "NOTE", "", ""], rows)
})

const homeFeed = Effect.gen(function* () {
  const summary = yield* Storage.use.snapshotSummary()
  const debug = yield* CurrentSiteDebug
  ...
  return document(summary, debug, siteKpis(history56), ["SIGNAL CATEGORY", "COUNT", "", "", "", ""], rows)
})

const opportunitiesFeed = Effect.gen(function* () {
  const summary = yield* Storage.use.snapshotSummary()
  const debug = yield* CurrentSiteDebug
  const origin = yield* CurrentSiteOrigin
  const registry = yield* Registry.use.loadRegistry()
  ...
  const rows: FeedRow[] = digest.signals.map((signal, index) => {
    ...
    return {
      ...
      nodes: [
        ...
        prose(signalReason(signal, origin)),
        sect("Recommended action"),
...
const historyFeed = Effect.gen(function* () {
  ...
  return document(summary, debug, meta, ["DATE", "IMPR", "CHANGE", "CLICKS", "CTR", "POS"], rows)
})

const registryFeed = Effect.gen(function* () {
  const summary = yield* Storage.use.snapshotSummary()
  const debug = yield* CurrentSiteDebug
  const origin = yield* CurrentSiteOrigin
  const registry = yield* Registry.use.loadRegistry()
  ...
  for (const progress of targets) {
    ...
    const performance = yield* Storage.use.targetPerformance(progress.targetUrl, inventoryOnly)
      ...
      nodes: [
        ...
        ...(activity.length > 0
          ? activity.map((logEntry) => {
              const compact = compactReadout(logEntry.readout)
              return item(`${shortDate(logEntry.date)} · ${logKindLabel(logEntry.kind)}${logEntry.note ? ` — ${logEntry.note}` : ""}${compact ? ` · ${compact}` : ""}`)
            })
  ...
  return document(summary, debug, meta, ["PRIORITY", "TARGET URL", "KW", "IMPR", "CLICKS", "PHASE"], rows)
})

// Small shared reads — the active site's origin and the debug flag.
const CurrentSiteOrigin = Effect.map(CurrentSite.use.current(), (site) => site.origin)
const CurrentSiteDebug = Effect.gen(function* () {
  const path = yield* CurrentSite.use.databasePath()
  return path.endsWith(".debug.sqlite")
})

export const feedFor = (view: FeedView) => {
  switch (view) {
    case "home":
      return homeFeed
    case "opportunities":
