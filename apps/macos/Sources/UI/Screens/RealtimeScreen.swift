...
/// The realtime: every site at once, live, on the same page as a site's own tab. The header
...
/// first. This is the one tab that polls. Search Console has its figures on each site's
/// tab, and the sites compared over a stored period are the Overview's.
///
...
struct RealtimeScreen: View {
    ...
    @Bindable var state: RealtimeTabState
    let live: LiveStore
...
private struct RealtimeStrip: View {
    let sites: [Site]
    let live: LiveStore
    ...
    var body: some View {
        ...
        HStack(alignment: .top, spacing: 0) {
            Metric(title: "Online", value: RealtimeScreen.count(online), dot: visitsColor)
            gap
            Metric(title: "Visits today", value: RealtimeScreen.count(visits))
            gap
            Metric(title: "Pageviews today", value: RealtimeScreen.count(pageviews))
            gap
            Metric(title: "Events today", value: RealtimeScreen.count(events))
            gap
            Rectangle()
                .fill(Palette.line)
...
private struct SiteTile: View {
    ...
    @ViewBuilder private var trailing: some View {
        if let error = overview.errorMessage {
            ...
            Text("\(RealtimeScreen.count(today.site?.visits)) today")
                .foregroundStyle(.secondary)
                .help("\(RealtimeScreen.count(today.site?.visits)) visits and \(RealtimeScreen.count(today.eventCount)) events today")
        }
    }
    ...
    @ViewBuilder private var hero: some View {
        if let live {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text(RealtimeScreen.count(live.onlineNow))
